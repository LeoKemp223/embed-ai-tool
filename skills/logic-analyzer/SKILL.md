---
name: logic-analyzer
description: 当需要用 Saleae 逻辑分析仪在线采集数字波形并解码 I2C/SPI/UART 时使用，通过 Logic 2 自动化服务驱动硬件采集、添加协议解码器并导出数据。
---
...
支持新协议（如 CAN、1-Wire、Parallel）只需在 [scripts/la_tool.py](scripts/la_tool.py) 的 `ANALYZER_SPECS` 字典加一条：协议短名 → Logic 2 解码器名（`ANALYZER_NAMES`）、短键到 settings 键的映射、必填键列表。解析、采集、导出逻辑全部自动复用，无需改函数体。
